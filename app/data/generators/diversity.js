...
module.exports = () => {
  ...
  const hasDisability = weighted.select([true, false], [0.3, 0.7])

  const disabilityCount = faker.helpers.shuffle([1, 2, 3])[0] // up to 3 disabilities

  const disabilityChoices = disabilities.items.map(item => item.value)

  const shuffledDisabilities = faker.helpers.shuffle(disabilityChoices)
