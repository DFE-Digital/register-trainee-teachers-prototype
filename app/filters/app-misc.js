...
filters.getAcademicQualificationText = function (input, record = false) {
  ...
  // QTS or EYTS
  const qualificationText = utils.getQualificationText(record) || 'QTS'
  // Whether a route has academic qualifications (all except Assessment only)
  const qualificationsApply = utils.academicQualificationsApply(record)

  const foundQualification = utils.lookUpAcademicQualification(input)
...
filters.getSchoolHint = (school) => {
  const urn = school?.urn ? `URN ${school.urn}` : false
  const address = school?.town ? school.town : [school?.addressLine1, school?.addressLine2].filter(Boolean).join(', ')
  const items = [urn, address, school?.postcode].filter(Boolean)
  return items.join(', ')
}
