...
// Settings
const simpleGcseGrades = true // output pass/fail rather than full detail
...
const generateFakeApplication = (params = {}) => {
  ...
  if (!application.academicYear) {
    const startYear = params.academicYearSimple || currentYear
  ...
  const nationality = application?.personalDetails?.nationality
  ...
  const requiredSections = trainingRouteData.trainingRoutes[application.route].sections
  ...
  const routeQualifications = trainingRouteData.trainingRoutes[application.route].qualifications
...
const generateFakeApplicationsForProvider = (provider, year, count) => {
  ...
  const applyStubUpdatedDate = faker.date.between(
    moment().subtract(16, 'days'),
    moment()
  )
  ...
  for (let i = 0; i < count; i++) {
    ...
    const statusPick = weighted.select(targetCounts)
