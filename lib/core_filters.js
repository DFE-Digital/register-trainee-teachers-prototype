module.exports = function (env) {
   ...
   */
  filters.log = function log (a, string) {
    if (string) {
      return nunjucksSafe('<script>console.log("' + string + ':", ' + JSON.stringify(a, null, '\t') + ');</script>')
    } else return nunjucksSafe('<script>console.log(' + JSON.stringify(a, null, '\t') + ');</script>')
  }
