...
const setSubjectSpecialisms = (courseDetails, pickRandom) => {
  ...
  Object.keys(courseDetails.publishSubjects).forEach(ordinal => {
    const theSubject = courseDetails.publishSubjects[ordinal]
    ...
    if (theSubject.includes('Primary')) {
      ...
      const randomLanguage = faker.helpers.arrayElement(['French language', 'Spanish language', 'German language', 'Modern languages'])
...
module.exports = (params, application) => {
  ...
  const pretendDataIsComplete = (isNonDraft || isManualDraft || sectionIsComplete)
  ...
  if (isPublishCourse) {
    ...
    const limitedCourses = routeCourses.slice(0, 12) // to match data.settings.courseLimit
    ...
    if (pretendDataIsComplete) {
      ...
      const startDateVague = moment(courseDetails.startDateVague).toDate()
      const startMonth = startDateVague.getMonth() + 1 // Month 0 is Jan
      const startYear = startDateVague.getFullYear()
