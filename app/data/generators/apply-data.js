...
const pickRandomConditions = () => {
  const numberOfConditions = getRandomInt(1, 4)

  console.log(`Number of conditions: ${numberOfConditions}`)

  const copyOfConditions = Object.assign({}, commonApplyConditions)

  const getRandomCondition = (conditions) => {
    return weighted.select(conditions)
  }

  const selectedConditions = Array(numberOfConditions).fill().map((item, index) => {
    const selectedCondition = getRandomCondition(copyOfConditions)
